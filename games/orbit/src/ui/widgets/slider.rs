...
fn on_add_slider(mut world: DeferredWorld<'_>, HookContext { entity, .. }: HookContext) {
    ...
    world.commands().entity(entity)
        .observe(update_colors_on::<Pointer<Over>>(HOVERED_SLIDER_TRACK, HOVERED_SLIDER_TRACK))
        .observe(update_colors_on::<Pointer<Out>>(SLIDER_TRACK_COLOR, SLIDER_TRACK_COLOR));
...
fn on_add_slider_thumb(mut world: DeferredWorld<'_>, HookContext { entity, .. }: HookContext) {
    ...
    world.commands().entity(entity)
        .observe(update_colors_on::<Pointer<Over>>(HOVERED_SLIDER_THUMB, HOVERED_SLIDER_THUMB))
        .observe(update_colors_on::<Pointer<Out>>(SLIDER_THUMB_COLOR, SLIDER_THUMB_COLOR))
        .observe(update_colors_on::<Pointer<Pressed>>(SLIDER_THUMB_COLOR, SLIDER_THUMB_COLOR))
        .observe(update_colors_on::<Pointer<Released>>(HOVERED_SLIDER_THUMB, HOVERED_SLIDER_THUMB))
...
fn update_slider_value_on_drag(
    ...
    let slider_entity = child_of.parent;
        ...
        let (node, mut slider_value) = slider_query.get_mut(slider_entity).unwrap();
            ...
            let track_width = if let Val::Px(w) = node.width { w } else { 300.0 };
